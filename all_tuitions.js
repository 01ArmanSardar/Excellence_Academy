const loadTuition = (search) => {
    ...
    fetch(`https://excileence-academy-api-5v91.onrender.com/tution/?search=${search ? search :""}`)
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
        if (data.length > 0) {
  ...
  const loadTuition_by_filter = (search) => {
    document.getElementById("tuition-container").innerHTML = "";
    fetch(`https://excileence-academy-api-5v91.onrender.com/tuition/?search=${search ? search :""}`)
      .then((res) => res.json())
      .then((data) => {
